  ...
  <div class="account-page page-container">
    ...
    <div class="page-card">
      <el-table :data="accounts" border stripe style="width: 100%" v-loading="loading" :header-cell-style="{ background: 'var(--bg-color)', color: 'var(--text-color)' }">
        ...
        <el-table-column label="教师姓名" min-width="100">
          <template #default="{ row }">{{ row.detail?.teacher_name ?? '-' }}</template>
        </el-table-column>
        <el-table-column label="科目" min-width="80" align="center">
...
import { getTeacherAccounts, resetAccountPassword, type AccountInfo } from '@/api/account'
...
async function fetchAccounts() {
  loading.value = true
  try {
    const res = await getTeacherAccounts()
    if (res?.data) accounts.value = res.data as unknown as AccountInfo[]
  } catch (e) { console.error('获取教师账号失败:', e) } finally { loading.value = false }
}
