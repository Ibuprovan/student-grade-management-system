<template>
  <div class="account-page page-container">
    <div class="page-header">
      <h1 class="page-title">学科组长账号管理</h1>
    ...
    <div class="page-card">
      <el-table :data="accounts" border stripe style="width: 100%" v-loading="loading" :header-cell-style="{ background: 'var(--bg-color)', color: 'var(--text-color)' }">
        ...
        <el-table-column label="学科" min-width="90">
          <template #default="{ row }">{{ row.detail?.subject ?? '-' }}</template>
        </el-table-column>
        <el-table-column label="英文名" min-width="100">
          <template #default="{ row }">{{ row.detail?.subject_en ?? '-' }}</template>
        </el-table-column>
        <el-table-column label="组长姓名" min-width="100">
          <template #default="{ row }">{{ row.detail?.leader_name ?? '-' }}</template>
        </el-table-column>
...
import { getSubjectLeaderAccounts, resetAccountPassword, type AccountInfo } from '@/api/account'
...
async function fetchAccounts() {
  loading.value = true
  try {
    const res = await getSubjectLeaderAccounts()
    if (res?.data) accounts.value = res.data as unknown as AccountInfo[]
  } catch (e) { console.error('获取学科组长账号失败:', e) } finally { loading.value = false }
}
