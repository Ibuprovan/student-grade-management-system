  ...
  <div class="account-page page-container">
    ...
    <div class="page-card">
      <el-table :data="accounts" border stripe style="width: 100%" v-loading="loading" :header-cell-style="{ background: 'var(--bg-color)', color: 'var(--text-color)' }">
        <el-table-column prop="username" label="用户名（学号）" min-width="130" />
        <el-table-column label="姓名" min-width="100">
          <template #default="{ row }">{{ row.detail?.name ?? '-' }}</template>
        </el-table-column>
        ...
        <el-table-column label="性别" min-width="70" align="center">
          <template #default="{ row }">{{ row.detail?.gender ?? '-' }}</template>
        </el-table-column>
...
import { getStudentAccounts, resetAccountPassword, type AccountInfo } from '@/api/account'
...
async function fetchAccounts() {
  loading.value = true
  try {
    const res = await getStudentAccounts()
    if (res?.data) accounts.value = res.data as unknown as AccountInfo[]
  } catch (e) { console.error('获取学生账号失败:', e) } finally { loading.value = false }
}
