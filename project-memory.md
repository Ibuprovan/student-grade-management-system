...
- **UI组件库**: Element Plus 2.4+（V3.0 Supabase风格主题覆盖）
- **图表库**: ECharts 5.4+
- **状态管理**: Pinia 2.1+
- **构建工具**: Vite 4.4+
- **CSS预处理**: SCSS（V3.0设计令牌系统）
- **前端测试**: Vitest 0.34+
...
│   ├── assets/
│   │   ├── styles/
│   │   │   ├── variables.scss       # 设计令牌变量（V3.0新增）
│   │   │   ├── element-override.scss # Element Plus样式覆盖（V3.0新增）
│   │   │   └── global.scss          # 全局样式（V3.0重写）
│   │   └── images/
│   ├── components/      # 公共组件
│   │   └── layout/
│   │       ├── AppSidebar.vue # 侧边栏（V3.0 Supabase风格）
│   │       ├── AppHeader.vue  # 头部导航（V3.0 Supabase风格）
│   │       └── AppLayout.vue  # 整体布局（V3.0 Supabase风格）
│   ├── directives/      # 自定义指令
│   │   └── permission.ts # 权限指令
│   ├── views/           # 页面组件
│   │   ├── login/       # 登录页面
